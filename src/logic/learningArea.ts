...
export const getLearningArea = async (req: Request<{}, {}, learningAreaFields>, res: Response) => {
    ...
    query += 'SELECT * FROM learning_area la \n'
...
export const createLearningArea = async (req: Request<{}, {}, learningAreaFields>, res: Response) => {
    ...
    if (!inst_id || !learning_area_name || typeof flag_valid !== "boolean") {
        res.status(400).json({ success: false, message: "Missing required fields!" });
        return;
    }

    const query = `INSERT INTO learning_area (inst_id, learning_area_name, remark, flag_valid)
                   VALUES ($1, $2, $3, $4)`;
    const values = [inst_id, learning_area_name, remark, flag_valid];
